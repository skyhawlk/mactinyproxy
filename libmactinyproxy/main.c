...
int start (void* args)
{
        int opt, daemonized = FALSE;
    
    
        startverbs *verbs = (startverbs *) args;
...
*/
        ...
        if (set_signal_handler (SIGPIPE, SIG_IGN) == SIG_ERR) {
                fprintf (stderr, "Could not set the \"SIGPIPE\" signal.\n");
                exit (EX_OSERR);
        }
        ...
        if (config->filter)
                filter_init ();
#endif /* FILTER_ENABLE */
        proxy_filterurl_cb_register(verbs->cb);
    
        /* Start listening on the selected port. */
        if (child_listening_sockets(config->listen_addrs, config->port) < 0) {
                fprintf (stderr, "Could not create listening sockets.\n"
                        );
                exit (EX_OSERR);
        }
